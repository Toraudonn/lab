...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            # print(env.is_episode_finished())
            env.new_episode()
            ob = process_frame(env.get_state().screen_buffer)
            print("doesn't come here")
            remote.send(ob)
        elif cmd == 'reset_task':
...
class VecEnv():
    def __init__(self, env_fns, logging=False, log_dir='/tmp/vizdoom/'):
        ...
        self.closed = False
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe(duplex=True) for _ in range(nenvs)])
        print(self.remotes)
        ...
        arr = self.remotes[0].recv()
        print(arr)
        self.action_space_shape, self.observation_space_shape = arr
